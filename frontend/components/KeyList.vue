...
<template>
  <div class="flex flex-col h-full bg-white dark:bg-gray-800">
    ...
    <div class="p-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex justify-between items-center">
      <div class="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-200 truncate" :title="`${connection.host}:${connection.port}`">
        <div class="w-2 h-2 rounded-full bg-green-500 flex-shrink-0"></div>
        <span class="truncate">{{ connection.host }}:{{ connection.port }}</span>
      </div>
      <div class="flex items-center gap-1">
        <button class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 p-1" :title="isDark ? 'Switch to Light Mode' : 'Switch to Dark Mode'" @click="toggleTheme">
    ...
    <div class="flex-1 overflow-y-auto">
      ...
      <div v-else-if="keys.length === 0" class="p-4 text-center text-gray-500 dark:text-gray-400 text-sm">No keys found.</div>
      ...
      <template v-else>
        ...
        <div v-if="hasMore" class="p-4 text-center">
          <button class="text-xs text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 font-medium disabled:opacity-50" :disabled="loading" @click="loadMore">
